...
const (
	// CashAddr/Bech32 AddressFormat is the DeVault standard
	CashAddr
)

const (
	// address prefixes
	MainNetPrefix = "devault:"
	TestNetPrefix = "dvtest:"
	RegTestPrefix = "dvreg:"
)
...
func init() {
	...
	MainNetParams.Net = dvtutil.MainnetMagic

	TestNetParams = chaincfg.TestNet3Params
	TestNetParams.Net = dvtutil.TestnetMagic

	RegtestParams = chaincfg.RegressionNetParams
	RegtestParams.Net = dvtutil.Regtestmagic
}

// DevaultParser handle
type DevaultParser struct {
	*btc.BitcoinParser
	AddressFormat AddressFormat
}

// NewDevaultParser returns new DevaultParser instance
func NewDevaultParser(params *chaincfg.Params, c *btc.Configuration) (*DevaultParser, error) {
...
func (p *DevaultParser) GetAddrDescFromAddress(address string) (bchain.AddressDescriptor, error) {
	return p.addressToOutputScript(address)
}

// addressToOutputScript converts bitcoin address to ScriptPubKey
func (p *DevaultParser) addressToOutputScript(address string) ([]byte, error) {
		da, err := dvtutil.DecodeAddress(address, p.Params)
		if err != nil {
			return nil, err
		}
		script, err := dvtutil.PayToAddrScript(da)
		if err != nil {
			return nil, err
		}
		return script, nil
...
func (p *DevaultParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
	// convert possible P2PK script to P2PK, which dvtutil can process
	var err error
	script, err = txscript.ConvertP2PKtoP2PKH(script)
	if err != nil {
		return nil, false, err
	}
	a, err := dvtutil.ExtractPkScriptAddrs(script, p.Params)
	if err != nil {
