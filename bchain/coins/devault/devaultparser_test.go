...
func setupParsers(t *testing.T) (mainParserCashAddr, mainParserLegacy, testParserCashAddr, testParserLegacy *DeVaultParser) {
	parser1, err := NewDeVaultParser(GetChainParams("main"), &btc.Configuration{AddressFormat: "cashaddr"})
	if err != nil {
		t.Fatalf("NewDeVaultParser() error = %v", err)
	}
	parser2, err := NewDeVaultParser(GetChainParams("main"), &btc.Configuration{AddressFormat: "legacy"})
	if err != nil {
		t.Fatalf("NewDeVaultParser() error = %v", err)
	}
	parser3, err := NewDeVaultParser(GetChainParams("test"), &btc.Configuration{AddressFormat: "cashaddr"})
	if err != nil {
		t.Fatalf("NewDeVaultParser() error = %v", err)
	}
	parser4, err := NewDeVaultParser(GetChainParams("test"), &btc.Configuration{AddressFormat: "legacy"})
	if err != nil {
		t.Fatalf("NewDeVaultParser() error = %v", err)
	}
